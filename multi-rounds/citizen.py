    ...
    def update_belief_after_dialogue(self, belief_change, conversation_history):
        ...
        if new_belief_value != current_belief:
            ...
            if new_belief_value == 1:
                new_opinion = f"我现在相信：{random.choice(topic_sentence_infeted)}"
            else:
                new_opinion = f"我现在不相信：{random.choice(topic_sentence_susceptible)}"
            ...
            # 信念未变，保持原有观点但可能有细微调整
            if current_belief == 1:
                new_opinion = f"我依然相信：{random.choice(topic_sentence_infeted)}"
            else:
                new_opinion = f"我依然不相信：{random.choice(topic_sentence_susceptible)}"
    ...
    def reflect_on_community(self, community_opinions):
        ...
        try:
            ...
            if updated_belief != self.beliefs[-1]:
                if updated_belief == 1:
                    new_opinion = f"经过反思，我相信：{random.choice(topic_sentence_infeted)}"
                else:
                    new_opinion = f"经过反思，我不相信：{random.choice(topic_sentence_susceptible)}"
